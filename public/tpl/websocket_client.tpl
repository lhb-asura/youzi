...
<head>
    ...
    <style type="text/css">
        ...
        #sd{float:right; width:80px; height:35px; font-size:18px;}
...
<div class="rin">
    <button id="sd" {if !session("?user")}hidden{/if}>发送</button>
  <!--  <span><img src="http://www.yxsss.com/ui/sk/t.png" title="表情" id="imgbq"><img src="http://www.yxsss.com/ui/sk/e.png" title="上传图片">
        <form>
            <!--<input type="file" title="上传图片" id="upimg"></form></span>-->
    {if !session("?user")}<span style="font-size: 20px">您还未
        <a style="color: #31A030;cursor: pointer;"  class="button cur" data-toggle="modal" data-target="#myModal">登录</a></span>{/if}
    <p><input id="nrong"></p>
</div>
...
<script>
    (function(){
        ...
        function st(){
            n="{if session("?user")}{:session('user')['name']}{else /}{$socket_name}{/if}";
          //  n=n.substr(0,16);
            if(!n){
                return ;
            }
            ...
            so.onclose=function(){
                so=false;
                lct.appendChild(A.$$('<p class="c2">退出</p>'));
            }
